...
public class KickoutSessionControlFilter extends AccessControlFilter {
	 ...
	 * 登录剔出
	 */
	private final static String LOGIN_KICKOUT = "kickout";
	...
	protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
		...
		String username = ((Customer) subject.getPrincipal()).getLogin();
		...
		if (!deque.contains(sessionId) && session.getAttribute(LOGIN_KICKOUT) == null) {
        ...
        if (session.getAttribute(LOGIN_KICKOUT) != null) {
            //会话被踢出了
            try {
                subject.logout();
            } catch (Exception e) {
